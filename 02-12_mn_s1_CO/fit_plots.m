...
title(sprintf('%s, obs minus calc all FOVs', pname));
...
title(sprintf('%s, obs minus calc FOV 5', pname));
...
xlabel('wavenumber')
ylabel('transmittance difference')
title(sprintf('%s, obs minus calc side FOVs', pname));
legend(fname{ix}, 'location', 'southeast')
grid on; zoom on
...
xlabel('wavenumber')
ylabel('transmittance difference')
title(sprintf('%s, obs minus calc corner FOVs', pname));
legend(fname{ix}, 'location', 'southeast')
grid on; zoom on
...
opt2 = struct;
opt2.resmode = opt.resmode;
[inst, user] = inst_params(band, wlaser, opt2);
